...
Runs on `data/candidates_small.jsonl` (default 500 rows) and performs:
- load candidates
- build candidate embeddings (semantic_match)
- build features (feature_engineering)
- rank candidates (ranker)
- write submission CSV and validate it

Writes runtime metrics to `scripts/e2e_small_metrics.json`.
"""
...
# Also add src directory so local imports like `import feature_engineering` resolve
src_dir = root / "src"
...
def main():
    ...
    cand_path = Path("data/candidates_small.jsonl")
    ...
    # Build embeddings (TF-IDF fallback will run quickly for sample)
    t = time.perf_counter()
    ...
    # Write CSV
    t = time.perf_counter()
    out_csv = Path("submission_top100_small.csv")
    ...
    metrics_path = Path("scripts/e2e_small_metrics.json")
