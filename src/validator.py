#!/usr/bin/env python3
"""Validate submission CSV per Redrob rules.

This module provides `validate_submission(csv_path)` returning a list of
error strings (empty list = valid). It mirrors the rules in
`data/validate_submission.py` so callers can validate programmatically.
"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python src/validator.py <submission.csv>")
        sys.exit(1)

    errs = validate_submission(sys.argv[1])
    if errs:
